...
* FileNameInPut:文件路径
*/
char* GetFileSHA1(char* buffer,int len,char* sha1)
{
    ...
    sha1_write (&ctx, (unsigned char *)buffer, len);
	/*if (ferror (fp))
	{
		printf("读取文件“%s”失败\n", FileNameInPut);
		return;
	...
	for ( i=0; i < 20; i++)
	{
		sprintf(sha1+2*i,"%02x",(unsigned char)ctx.buf[i]);
	}
    sha1[2*i] = '\0';
    return sha1;
}
