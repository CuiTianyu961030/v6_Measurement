...
def get_ip_by_all_ipv4(dns_df, ssl_df, http_df):
...
def get_CS_by_dns(dns_df, queries_dict, answers_dict, ip_version, query_num, response_num, total_num):
    ...
    if ip_version == 6:
        ...
        response_num = response_num + \
            spark.sql("select count(1) from DNS_ALL where response is not null").rdd.map(lambda row: row[0]).collect()[
                0]
        total_num = total_num + \
    ...
    cs_queries_count_df = cs_queries_df.groupBy("name").count()
    cs_answers_count_df = cs_answers_df.groupBy("name").count()
    if os.path.exists("E:\\tempfile\\DNS_queries"):
        files = os.listdir("E:\\tempfile\\DNS_queries")
        for afile in files:
            os.remove("E:\\tempfile\\DNS_queries\\" + afile)
        os.rmdir("E:\\tempfile\\DNS_queries")
    if os.path.exists("E:\\tempfile\\DNS_answers"):
        files = os.listdir("E:\\tempfile\\DNS_answers")
        for afile in files:
            os.remove("E:\\tempfile\\DNS_answers\\" + afile)
        os.rmdir("E:\\tempfile\\DNS_answers")
    cs_queries_count_df.rdd.map(lambda row: [row[0],row[1]]).saveAsTextFile("E:\\tempfile\\DNS_queries")
    cs_answers_count_df.rdd.map(lambda row: [row[0],row[1]]).saveAsTextFile("E:\\tempfile\\DNS_answers")
    #queries_list = cs_queries_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    #answers_list = cs_answers_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    #print queries_list
    #print answers_list
    #queries_dict = {}
    #answers_dict = {}
    #for element in queries_list:
    #    if queries_dict.has_key(element[0]):
    #        queries_dict[element[0]] = queries_dict.get(element[0]) + 1
    #    else:
    #        queries_dict[element[0]] = int(element[1])
    #for element in answers_list:
    #    if answers_dict.has_key(element[0]):
    #        answers_dict[element[0]] = answers_dict.get(element[0]) + 1
    #    else:
    #        answers_dict[element[0]] = int(element[1])
    #print queries_dict
    #print answers_dict
    #print os.listdir("E:\\tempfile\\DNS_queries")
    for dir in os.listdir("E:\\tempfile\\DNS_queries"):
        if ".crc" not in dir:
            if "_" not in dir:
                #print dir
                f = open("E:\\tempfile\\DNS_queries\\" + dir, "r")
                while 1:
                    lines = f.readlines(10000)
                    if not lines:
                        break
                    for line in lines:
                        if queries_dict.has_key(line[3:line.find(",")-1]):
                            queries_dict[line[3:line.find(",")-1]] = int(queries_dict[line[3:line.find(",")-1]]) + 1
                        else:
                            queries_dict[line[3:line.find(",")-1]] = int(line[line.find(",")+1:-2])
                f.close()
    for dir in os.listdir("E:\\tempfile\\DNS_answers"):
        if ".crc" not in dir:
            if "_" not in dir:
                f = open("E:\\tempfile\\DNS_answers\\" + dir, "r")
                while 1:
                    lines = f.readlines(10000)
                    if not lines:
                        break
                    for line in lines:
                        if answers_dict.has_key(line[3:line.find(",")-1]):
                            answers_dict[line[3:line.find(",")-1]] = int(answers_dict[line[3:line.find(",")-1]]) + 1
                        else:
                            answers_dict[line[3:line.find(",")-1]] = int(line[line.find(",")+1:-2])
                f.close()
    print queries_dict
    print answers_dict
    #cs_queries_count_df = cs_queries_count_df.where("count > 10000")
    #cs_answers_count_df = cs_answers_count_df.where("count > 10000")
    #cs_queries_count_df.createOrReplaceTempView("TEST")
    #cs_queries_count_df.rdd.map(lambda row: [row[0], row[1]]).saveAsTextFile("E:\\tempfile\\dns_queries")
    #cs_answers_count_df.rdd.map(lambda row: [row[0], row[1]]).saveAsTextFile("E:\\tempfile\\dns_answers")
    ...
    return(cs_queries_count_df, cs_answers_count_df, queries_dict, answers_dict, query_num, response_num, total_num)

def get_IPv6_stack_by_dns(dns_df, ip_version, geo4_count_dict, geo6_count_dict, geodual_count_dict, ipv6_name_dual_stack_count_dict):
    ...
    ipv6_name_dual_stack_count_df = ipv6_dual_stack_df.groupBy("name").count()
    ipv6_name_dual_stack_count_list = ipv6_name_dual_stack_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    for element in ipv6_name_dual_stack_count_list:
        if ipv6_name_dual_stack_count_dict.has_key(element[0]):
            ipv6_name_dual_stack_count_dict[element[0]] = ipv6_name_dual_stack_count_dict.get(element[0]) + 1
        else:
            ipv6_name_dual_stack_count_dict[element[0]] = int(element[1])
    ...
    geo4_count_list = geo4_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    for element in geo4_count_list:
        if geo4_count_dict.has_key(element[0]):
            geo4_count_dict[element[0]] = geo4_count_dict.get(element[0]) + 1
        else:
            geo4_count_dict[element[0]] = int(element[1])

    geo6_RDD = sc.parallelize(geo6_list)
    geo6_df = spark.read.json(geo6_RDD)
    #geo6_df.show()
    geo6_count_df = geo6_df.groupBy("country_code").count()
    geo6_count_list = geo4_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    for element in geo6_count_list:
        if geo6_count_dict.has_key(element[0]):
            geo6_count_dict[element[0]] = geo6_count_dict.get(element[0]) + 1
        else:
            geo6_count_dict[element[0]] = int(element[1])
    ...
    geodual_count_list = geo4_count_df.rdd.map(lambda row: [row[0], row[1]]).collect()
    for element in geodual_count_list:
        if geodual_count_dict.has_key(element[0]):
            geodual_count_dict[element[0]] = geodual_count_dict.get(element[0]) + 1
        else:
            geodual_count_dict[element[0]] = int(element[1])

    "### finish running Get_IPv6_Stack_By_DNS ###"
    return(dns_ipv6_stack_df, ipv6_dual_stack_df, geo4_count_df, geo6_count_df, geodual_count_df, geo4_count_dict, geo6_count_dict, geodual_count_dict)
...
def get_tunnel_by_all(dns_df, ssl_df, http_df):
    ...
    total_dns_tunnel_list = [[0, 0, 0, 0, 0, 0]]
    total_ssl_tunnel_list = [[0, 0, 0, 0, 0, 0]]
    total_http_tunnel_list = [[0, 0, 0, 0, 0, 0]]

    tunnel_dns_df = spark.sql("SELECT * FROM DNS_All WHERE sip is null")

    if str(dns_df.rdd.collect()).find("6in4=") != -1:
        tunnel_dns_6in4_df = spark.sql("SELECT * FROM DNS_All WHERE 6in4 is not null")
        total_dns_tunnel_list[0][0] = str(tunnel_dns_6in4_df.describe().rdd.map(lambda row: row[1]).collect()[0])
        #tunnel_dns_6in4_df.show()
        tunnel_dns_df = tunnel_dns_df.union(tunnel_dns_6in4_df)
    if str(dns_df.rdd.collect()).find("6to4=") != -1:
        tunnel_dns_6to4_df = spark.sql("SELECT * FROM DNS_All WHERE 6to4 is not null")
        total_dns_tunnel_list[0][1] = str(tunnel_dns_6to4_df.describe().rdd.map(lambda row: row[1]).collect()[0])
        #tunnel_dns_6to4_df.show()
        tunnel_dns_df = tunnel_dns_df.union(tunnel_dns_6to4_df)
    if str(dns_df.rdd.collect()).find("6over4=") != -1:
        tunnel_dns_6over4_df = spark.sql("SELECT * FROM DNS_All WHERE 6over4 is not null")
        total_dns_tunnel_list[0][2] = str(tunnel_dns_6over4_df.describe().rdd.map(lambda row: row[1]).collect()[0])
    ...
    tunnel_ssl_df = spark.sql("SELECT * FROM SSL_All WHERE sip is null")

    if str(ssl_df.rdd.collect()).find("6in4=") != -1:
        tunnel_ssl_6in4_df = spark.sql("SELECT * FROM SSL_All WHERE 6in4 is not null")
    ...
    if str(ssl_df.rdd.collect()).find("6to4=") != -1:
        tunnel_ssl_6to4_df = spark.sql("SELECT * FROM SSL_All WHERE 6to4 is not null")
    ...
    if str(ssl_df.rdd.collect()).find("6over4=") != -1:
        tunnel_ssl_6over4_df = spark.sql("SELECT * FROM SSL_All WHERE 6over4 is not null")
    ...
    if str(ssl_df.rdd.collect()).find("teredo=") != -1:
        tunnel_ssl_teredo_df = spark.sql("SELECT * FROM SSL_All WHERE teredo is not null")
    ...
    if str(ssl_df.rdd.collect()).find("ISATAP=") != -1:
        tunnel_ssl_ISATAP_df = spark.sql("SELECT * FROM SSL_All WHERE ISATAP is not null")
    ...
    if str(ssl_df.rdd.collect()).find("GRE=") != -1:
        tunnel_ssl_GRE_df = spark.sql("SELECT * FROM SSL_All WHERE GRE is not null")
    ...
    tunnel_http_df = spark.sql("SELECT * FROM HTTP_All WHERE sip is null")

    if str(http_df.rdd.collect()).find("6in4=") != -1:
        tunnel_http_6in4_df = spark.sql("SELECT * FROM HTTP_All WHERE 6in4 is not null")
    ...
    if str(http_df.rdd.collect()).find("6to4=") != -1:
        tunnel_http_6to4_df = spark.sql("SELECT * FROM HTTP_All WHERE 6to4 is not null")
    ...
    if str(http_df.rdd.collect()).find("6over4=") != -1:
        tunnel_http_6over4_df = spark.sql("SELECT * FROM HTTP_All WHERE 6over4 is not null")
    ...
    if str(http_df.rdd.collect()).find("teredo=") != -1:
        tunnel_http_teredo_df = spark.sql("SELECT * FROM HTTP_All WHERE teredo is not null")
    ...
    if str(http_df.rdd.collect()).find("ISATAP=") != -1:
        tunnel_http_ISATAP_df = spark.sql("SELECT * FROM HTTP_All WHERE ISATAP is not null")
    ...
    if str(http_df.rdd.collect()).find("GRE=") != -1:
        tunnel_http_GRE_df = spark.sql("SELECT * FROM HTTP_All WHERE GRE is not null")
    ...
    total_dns_tunnel_df = spark.createDataFrame(total_dns_tunnel_RDD, ["6in4", "6to4", "6over4", "teredo", "ISATAP", "GRE"])
    total_ssl_tunnel_df = spark.createDataFrame(total_ssl_tunnel_RDD, ["6in4", "6to4", "6over4", "teredo", "ISATAP", "GRE"])
    total_http_tunnel_df = spark.createDataFrame(total_http_tunnel_RDD, ["6in4", "6to4", "6over4", "teredo", "ISATAP", "GRE"])
    total_tunnel_df = spark.createDataFrame(total_tunnel_RDD, ["6in4", "6to4", "6over4", "teredo", "ISATAP", "GRE"])
...
def draw_geo_stack_server(ip_version, geo4_count_dict, geo6_count_dict, geodual_count_dict):
    if ip_version == 6:

        #geo4 = geo4_count_df.toPandas()
        geo4 = pd.DataFrame({"country_code": geo4_count_dict.keys(), "count": geo4_count_dict.values()})
        dark_lighten_style = LightenStyle('#004466')
        ...
        geo6 = pd.DataFrame({"country_code": geo6_count_dict.keys(), "count": geo6_count_dict.values()})
        dark_lighten_style = LightenStyle('#004466')
        worldmap_chart = pygal.maps.world.World(fill=True, style=dark_lighten_style, interpolate='cubic',
                                                human_readable=True)
        ...
        geodual = pd.DataFrame({"country_code": geodual_count_dict.keys(), "count": geodual_count_dict.values()})
        dark_lighten_style = LightenStyle('#004466')
        worldmap_chart = pygal.maps.world.World(fill=True, style=dark_lighten_style, interpolate='cubic',
                                                human_readable=True)
...
def draw_IPv6_dual_stack_count(ip_version, ipv6_name_dual_stack_count_dict):
    #ipv6_dual_stack_count_df = ipv6_dual_stack_df.groupBy("name").count()
    #dual_stack = ipv6_dual_stack_count_df.toPandas()
    dual_stack =pd.DataFrame({"name": ipv6_name_dual_stack_count_dict.keys(), "count": ipv6_name_dual_stack_count_dict.values()})
...
#def draw_dns_domain_name(cs_queries_count_df, cs_answers_count_df, ip_version, queries_dict, answers_dict):
def draw_dns_domain_name(ip_version, queries_dict, answers_dict):
    #dns_queries_domain_name = cs_queries_count_df.toPandas()
    dns_queries_domain_name = pd.DataFrame({"name": queries_dict.keys(), "count": queries_dict.values()})
    ...
    dns_answers_domain_name = pd.DataFrame({"name": answers_dict.keys(), "count": answers_dict.values()})
...
def draw_dns_detail(ip_version, query_num, response_num, total_num):
    if ip_version == 6:
        #query_num = \
        #spark.sql("select count(1) from DNS_ALL where query is not null").rdd.map(lambda row: row[0]).collect()[0]
        #response_num = \
...
def draw_ssl_certificate(ssl_certificate_df, ssl_certificate_issuer_count_df, ip_version):
    if ip_version == 6:
        ...
        xy_chart = pygal.XY(stroke=False, legend_at_bottom=True, human_readable=True)
...
def draw_http_cont_type(total_cont_type_df, cont_type_file_count_df, ip_version):
    if ip_version == 6:
        ...
        pie_chart = pygal.Pie(legend_at_bottom=True, human_readable=True)
...
def draw_tunnel(total_dns_tunnel_df, total_ssl_tunnel_df, total_http_tunnel_df, total_tunnel_df, ip_count_df):
    ...
    bar_chart.x_labels = map(str, ["6in4", "6to4", "6over4", "teredo", "ISATAP"])
    for index, row in dns_tunnel.iterrows():
        bar_chart.add("DNS", [int(row["6in4"]), int(row["6to4"]), int(row["6over4"]), int(row["teredo"]), int(row["ISATAP"])])
    for index, row in ssl_tunnel.iterrows():
        bar_chart.add("SSL", [int(row["6in4"]), int(row["6to4"]), int(row["6over4"]), int(row["teredo"]), int(row["ISATAP"])])
    for index, row in http_tunnel.iterrows():
        bar_chart.add("HTTP", [int(row["6in4"]), int(row["6to4"]), int(row["6over4"]), int(row["teredo"]), int(row["ISATAP"])])
    ...
    for index, row in total_tunnel.iterrows():
        tunnel_total = int(row["6in4"]) + int(row["6to4"]) + int(row["6over4"]) + int(row["teredo"]) + int(row["ISATAP"])
    ...
    print ip_total
    pie_chart_ip_tunnel = pygal.Pie(legend_at_bottom=True, human_readable=True)
    pie_chart_ip_tunnel.title = "IPv6 Tunnel Count in IPv6 Total Count Pie Chart"
    pie_chart_ip_tunnel.add("IPv6 Tunnel", tunnel_total * 100.0 /ip_total)
    pie_chart_ip_tunnel.add("IPv6 Total", 100.0 - tunnel_total * 100.0 / ip_total)
...
def draw_total_detail(ip_count_df, http_host_count_df, cs_ssl_count_df, cs_queries_count_df):
...
if __name__ == "__main__":
    ...
    geo6_count_dict = {}
    geodual_count_dict = {}
    ipv6_name_dual_stack_count_dict = {}
    num = 1
        ...
        for date_dir in date_dirs:
            ...
            for filename in filenames:
                ...
                # if protocol_dir == "tcp_udp":
                    #if filename == "tcp_udp_20180323_21":
                    # print file_path
                        # file_data = ""
                        # with open(file_path, "r") as f:
                        #     for line in f:
                        #         if "\x00" in line:
                        #         #if re.search("[\x00-\x1f]", line):
                        #             continue
                        #         else:
                        #             file_data += line
                        #     print file_data
                ...
                #if protocol_dir == "dns":
                #    print file_path
                    #if filename == "dns_20180323_01":
                    #if date_dir == "20180323":
                        #dns_df = spark.read.json(file_path)
                        #dns_df.createOrReplaceTempView("DNS_All")
                        #cs_queries_count_df, cs_answers_count_df, queries_dict, answers_dict, query_num, response_num, total_num = \
                            #get_CS_by_dns(dns_df, queries_dict, answers_dict, 6, query_num, response_num, total_num)
                    # dns_ipv6_stack_df, ipv6_dual_stack_df, geo4_count_df, geo6_count_df, geodual_count_df, \
                    # geo4_count_dict, geo6_count_dict, geodual_count_dict = get_IPv6_stack_by_dns( \
                    #    dns_df, 6, geo4_count_dict, geo6_count_dict, geodual_count_dict, ipv6_name_dual_stack_count_dict)
                #    dns_df = spark.read.json(file_path)
                #    test_file = dns_df.toPandas()
                #    print num
                #    num = num + 1
                ...
                #    test_file = http_df.toPandas()
                        ...
                        #http_df.show()
                        #http_server_count_df, http_host_count_df, http_user_agent_count_df, cs_http_df = get_CS_by_http(
                        #    http_df)
    ...
    #draw_dns_domain_name(6, queries_dict, answers_dict)
    #draw_geo_stack_server(6, geo4_count_dict, geo6_count_dict, geodual_count_dict)
    #draw_IPv6_dual_stack_count(6, ipv6_name_dual_stack_count_dict)
    #draw_dns_detail(6, query_num, response_num, total_num)
    #draw_dns_detail(4, query_num, response_num, total_num)
    ...
    #ipv4_ip_list, ipv4_ip_count_df = get_ip_by_all_ipv4(ipv4_dns_df, ipv4_ssl_df, ipv4_http_df)
    ...
    #cs_queries_count_df, cs_answers_count_df, queries_dict, answers_dict= get_CS_by_dns(dns_df, 6)
    #ipv4_cs_queries_count_df, ipv4_cs_answers_count_df = get_CS_by_dns(ipv4_dns_df, 4)

    #draw_dns_domain_name(cs_queries_count_df, cs_answers_count_df, 6, queries_dict, answers_dict)
'''
    cs_ssl_df, cs_ssl_count_df = get_CS_by_ssl(ssl_df, 6)
    ipv4_cs_ssl_df, ipv4_cs_ssl_count_df = get_CS_by_ssl(ipv4_ssl_df, 4)

    http_server_count_df, http_host_count_df, http_user_agent_count_df, cs_http_df = get_CS_by_http(http_df)
    ipv4_http_server_count_df, ipv4_http_host_count_df, ipv4_http_user_agent_count_df, ipv4_cs_http_df = get_CS_by_http(ipv4_http_df)
    ...
    #draw_ssl_certificate(ipv4_ssl_certificate_df, ipv4_ssl_certificate_issuer_count_df, 6)

    #draw_tunnel(total_dns_tunnel_df, total_ssl_tunnel_df, total_http_tunnel_df, total_tunnel_df, ip_count_df)
    ...
    #draw_IPv6_dual_stack_count(ipv4_dual_stack_df, 4)

    draw_total_detail(ip_count_df, http_host_count_df, cs_ssl_count_df, cs_queries_count_df)
